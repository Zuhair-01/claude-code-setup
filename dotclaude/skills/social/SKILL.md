...
**No video-gen credits available?** Use `free-ugc-pipeline` — a zero-cost fallback (free image gen + local TTS + Remotion compositing) that still applies this section's hook/structure/caption rules, for when the paid AI-video pipeline (`ugc-video-auto`, `seedance-2`) isn't usable.
