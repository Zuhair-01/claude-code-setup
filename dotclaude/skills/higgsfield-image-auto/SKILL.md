...
### Step 3: Clear the prompt bar, then type the prompt
...
confirm it's empty.** Skipping this risks the new prompt concatenating
onto whatever was left from a prior generation — a garbled prompt still
spends a real generation.

```
browser_evaluate → clear the textbox value via JS (not backspace/select-all,
  which can be unreliable on this component)
browser_take_screenshot → confirm the textbox is visibly empty before proceeding
```
...
Per standing project rule: for any batch of image generations (not a
single one-off), maintain `SESSION-RESUME.md` at the project root —
prompt queue, which ones are done, which failed, current position. A
browser crash or Playwright disconnect mid-batch shouldn't mean starting
