...
### Beyond This Skill (off-context deep references)

This skill covers tokens, perf/a11y/SSR guardrails, and the core interaction patterns (button,
modal, stagger, scroll parallax, skeleton, shared layout). For techniques below this line, pull the
named off-context skill via `python3 ~/.claude/overseer/search.py <name>` rather than duplicating
hundreds of lines of code here:

* **`framer-motion`** — Disney's 12 classical animation principles (squash/stretch, anticipation,
  follow-through, arc, secondary action, timing, exaggeration) mapped onto Motion code. Use when an
  animation feels mechanically correct but emotionally flat.
* **`motion-advanced`** — gestures and interaction primitives not covered above: draggable cards,
  drag-to-dismiss sheets, reorderable lists, swipe detection, long-press, `useMotionValue`/
  `useTransform`/`useAnimate` imperative sequences, SVG path draw-on, stroke progress rings, number
  counters, word-by-word text reveal, cursor followers.
* **`motion-patterns`** — additional production UI patterns: toast stacks, Next.js App Router page
  transitions, scroll progress bars, expanding cards, accordions, shared-element crossfade.
* **`motion-foundations`** — mostly redundant with this skill's Motion Tokens/Performance/
  Accessibility/SSR sections; only pull it if you need its exact spring-preset token values verbatim.
* **Non-React/3D animation:** `animejs-animation` (vanilla JS, no React dependency) and
  `threejs-animation` (3D keyframe/skeletal/GLTF animation) for work outside the Motion/React stack.
