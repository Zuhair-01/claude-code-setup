...
> **⚠️ Environment mismatch — the `"$OD_NODE_BIN" "$OD_BIN" media generate`
> dispatcher below is not installed in this Empire Base setup.** This
> skill was imported from a generic template that assumes an "OD" media
> CLI. Your actual short-form video pipeline is Playwright browser
> automation on higgsfield.ai — see `seedance-auto-generate` (video) and
> `higgsfield-image-auto` (image) instead. Keep this file for the shot
> planning / prompt-structure guidance only; the dispatch commands here
> will fail if run as-is.
