...
```sql
-- Window function: rank within groups without collapsing rows (unlike GROUP BY)
SELECT
...
-- Recursive CTE: walk a hierarchy (org chart, category tree) without app-side recursion
...
Pitfall: a recursive CTE with no terminating condition (or a cyclic manager_id reference) will
loop until the database's recursion-depth limit kills the query — always add a `depth` guard
(`WHERE depth < 50`) in the recursive branch for data you don't fully trust to be acyclic.
