...
name: prompt-master
version: 1.7.0
description: Generates optimized prompts for AI tools. On this account it also runs as an always-on internal filter (see "Account-Specific: Always-On Mode" below) — every user message gets silently sharpened through this skill's intent-extraction/Claude-routing logic before Claude acts on it, wired via a UserPromptSubmit hook (~/.claude/overseer/prompt_master_gate.py). The upstream default below (explicit-request-only) still governs when the user names an external tool.
---

## Account-Specific: Always-On Mode (added 2026-08-18 at Zoher's request)

On this account, prompt-master is **not** explicit-request-only — it is the mandatory first
filter between Zoher's raw message and any Claude response, enforced every turn by a
`UserPromptSubmit` hook. Two distinct modes, both live:

1. **Internal sharpening (default, silent)**: no external tool named → treat the raw message as
   the rough idea, run the Intent Extraction + "Claude (claude.ai, Claude API, Claude 4.x)"
   routing section below against it, then respond to the sharpened brief. Do **not** show a
   "paste this elsewhere" block — there's nowhere else to paste it, this session already is the
   target. Skip only for trivial one-line acknowledgements ("yes", "ok", "continue").

   **This is the core job, not an edge case**: Zoher's raw messages are frequently unclear,
   fragmented, typo-heavy, in caps, or mixing multiple asks in one line — that messiness is
   exactly what this filter exists to fix. Before responding:
   - Reconstruct the actual task in clear, professional language — what specifically is being
     asked for, on what target (file/skill/system/topic), with what scope.
   - Resolve ambiguous pronouns/references ("this", "it", "that thing") against the last few
     turns of context rather than guessing cold.
   - If the message bundles multiple asks, separate them and note the order you'll address them
     in, rather than picking one and dropping the rest.
   - If something is genuinely unresolvable even with context (a name/target that could mean two
     different things), ask ONE tight clarifying question rather than guessing wrong and doing
     the wrong work — per the skill's 3-question cap below.
   - The goal is Claude acting on the *precise* professional version of the request, every time,
     not a best-effort guess at a garbled one.
2. **External-tool prompt writing (explicit, as originally designed)**: user explicitly asks for
   a prompt to hand to another tool (Midjourney, Cursor, a different LLM, etc.) → follow the
   skill exactly as documented below, output format and all.

If ever in doubt which mode applies, default to (1) — silent sharpening — since that's the
steady-state behavior Zoher asked for; only switch to (2) when an external tool is clearly named.
