...
For deep SwiftUI implementation patterns/code, use `swiftui-patterns` instead — use this skill for architecture-level Apple-platform decisions.
...
```swift
// SwiftData (iOS 17+): replaces Core Data boilerplate for new projects
@Model
final class Note {
    var title: String
    var createdAt: Date
...
struct NoteListView: View {
    ...
    var body: some View {
        List(notes) { Text($0.title) }
            .toolbar { Button("Add") { context.insert(Note(title: "New")) } }
    }
}
```

Pitfall: `@Query` re-runs on every model context change in the whole app, not just this view's
model type -- for large datasets, add an explicit `#Predicate` filter rather than filtering the
fetched array in Swift, or the unfiltered fetch still happens every time.
...
```swift
// URLSession + async/await + Codable -- the standard modern pattern, no third-party dep needed
struct APIClient {
...
// Call site
let user: User = try await APIClient().fetch(url)
```

Pitfall: `URLSession.shared.data(from:)` throws on transport errors (no connection, timeout) but
does NOT throw on 4xx/5xx HTTP responses -- always check `response.statusCode` explicitly, a
`try await` succeeding is not proof the request succeeded.
