...
**For resize/speed/compress/convert/overlay/platform-export recipes**, use the **`ffmpeg`** skill —
it's the canonical deep FFmpeg reference (platform export presets, Remotion-prep patterns,
troubleshooting, batch multi-platform export script). This Layer 3 section only covers the
pipeline-specific operations above (EDL-driven batch cutting, proxy generation, transcription-ready
audio extraction, loudness normalization) that aren't in the general `ffmpeg` skill.
