...
## Lending/Borrowing Pool (Minimal)
...
// Overcollateralized lending: deposit collateral, borrow against it, liquidate if undercollateralized.
// This is a minimal skeleton -- production lending (Aave/Compound-style) needs interest-rate
// curves, oracle price feeds, and per-asset risk parameters on top of this.
contract SimpleLendingPool is ReentrancyGuard {
    IERC20 public collateralToken;
    IERC20 public borrowToken;

    uint256 public constant COLLATERAL_RATIO = 150; // 150% - must overcollateralize
    uint256 public constant LIQUIDATION_THRESHOLD = 120; // liquidatable below 120%
    ...
    function borrow(uint256 amount) external nonReentrant {
        uint256 newBorrow = borrowBalance[msg.sender] + amount;
        // 1:1 price assumed here -- production code MUST use an oracle, not a fixed ratio
        require(
            (collateralBalance[msg.sender] * 100) / newBorrow >= COLLATERAL_RATIO,
            "Insufficient collateral"
        );
        borrowBalance[msg.sender] = newBorrow;
    ...
    function healthFactor(address user) public view returns (uint256) {
        ...
        return (collateralBalance[user] * 100) / borrowBalance[user];
    }

    function liquidate(address user, uint256 debtToCover) external nonReentrant {
        require(healthFactor(user) < LIQUIDATION_THRESHOLD, "Position is healthy");
        borrowToken.transferFrom(msg.sender, address(this), debtToCover);
        borrowBalance[user] -= debtToCover;
        // 10% liquidation bonus paid in collateral -- incentivizes liquidators to act fast
        uint256 collateralSeized = (debtToCover * 110) / 100;
...
Pitfall: the `1:1 price assumed` line above is the single most exploited bug class in real DeFi
lending hacks — any production lending pool MUST price collateral and debt through a
manipulation-resistant oracle (Chainlink, or a TWAP over a deep liquidity pool), never a fixed
ratio or a single DEX spot price read in the same transaction (flash-loan price manipulation).
