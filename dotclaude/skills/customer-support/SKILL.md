...
**Concrete triage matrix** (adapt thresholds to actual SLA commitments, don't invent per-ticket):

| Signal | P1 (immediate) | P2 (same day) | P3 (48h) |
|---|---|---|---|
| Service down / data loss / security | Always | — | — |
| Paying customer, blocked from core workflow | If no workaround | If workaround exists | — |
| Sentiment: explicit anger/threat to churn | Escalate to human regardless of category | | |
| Feature request / "how do I" | — | — | Always |

**De-escalation response pattern** (acknowledge → own → concrete next step, in that order —
skipping straight to a fix without acknowledging frustration reads as dismissive):
> "That's frustrating, and I can see why — [restate their specific problem in one line]. I'm
> looking into it now and will have [a fix / a clear answer] by [concrete time], not just
> 'soon.'"

Pitfall: auto-escalating purely on negative-sentiment score without checking ticket category
floods human agents with low-stakes venting while genuinely urgent low-sentiment tickets (calm,
precise bug reports from technical users) get deprioritized — combine sentiment with impact
signals, never route on sentiment alone.
