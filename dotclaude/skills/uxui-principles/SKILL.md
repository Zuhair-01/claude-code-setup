...
## Related upstream catalogues (redirected here)
`frontend-design` and `ui-ux-pro-max` were separate thin catalogue entries pointing at different
upstream pattern-library repos (Anthropic's `frontend-design` and nextlevelbuilder's
`ui-ux-pro-max-skill`). Both cover the same ground as this skill's `uxui-evaluator` /
`interface-auditor` sub-skills with less structure, so this skill is now the canonical entry point
for "UI/UX pattern & heuristic" work — the two upstream links are preserved below for reference:
- https://github.com/anthropics/skills/tree/main/frontend-design
- https://github.com/nextlevelbuilder/ui-ux-pro-max-skill

For actual generative/opinionated visual taste (not evaluation/heuristics), use `taste-skill`
instead — this skill audits, `taste-skill` generates.
