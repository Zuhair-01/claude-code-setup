...
For deep Spring Boot implementation patterns/code, use `springboot-patterns` instead — use this skill for language/JVM-level decisions.
...
```java
// Virtual threads: one per request, no pool tuning needed (JDK 21+)
try (var executor = Executors.newVirtualThreadPerTaskExecutor()) {
  executor.submit(() -> fetchUser(id));
}

// Structured concurrency: fan-out two calls, fail fast if either fails (JDK 21 preview)
...
// Scoped values: safer than ThreadLocal under virtual threads (no leak across pooled threads)
static final ScopedValue<RequestContext> CTX = ScopedValue.newInstance();
ScopedValue.where(CTX, new RequestContext(traceId)).run(() -> handleRequest());
```

Pitfall: virtual threads are cheap but NOT for CPU-bound work — they still contend for the same
carrier-thread pool. Use them for blocking I/O (DB calls, HTTP calls), not tight compute loops.
...
```bash
# ZGC: sub-millisecond pause targets, good default for latency-sensitive services (JDK 21+)
java -XX:+UseZGC -XX:+ZGenerational -Xmx4g -jar app.jar

# G1: default GC, good throughput/latency balance for most services
java -XX:+UseG1GC -XX:MaxGCPauseMillis=200 -jar app.jar

# GraalVM native image: near-instant startup, ~10x lower memory -- trade slower build time
native-image -jar app.jar --no-fallback -O2

# JMH benchmark skeleton -- never trust microbenchmarks without JIT warmup
@BenchmarkMode(Mode.AverageTime)
@Warmup(iterations = 5) @Measurement(iterations = 5)
@State(Scope.Benchmark)
public class MyBenchmark {
  @Benchmark
  public int hotPath() { return compute(); }
}
```

Pitfall: don't switch GC based on a blog post — profile the actual workload first
(async-profiler flame graphs) and measure pause times under real load before tuning.
