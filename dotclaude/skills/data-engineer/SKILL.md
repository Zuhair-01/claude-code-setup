...
```python
# Airflow: idempotent, date-partitioned task -- reruns for the same date must be safe
...
def daily_orders_load():
    @task
    def extract(ds=None):
        return f"s3://raw/orders/{ds}/"  # ds = the logical date being processed, not "today"

    @task
    def load(path: str, ds=None):
        # DELETE + INSERT for this partition, not a blind append -- makes reruns idempotent
        run_sql(f"DELETE FROM orders WHERE load_date = '{ds}'; COPY orders FROM '{path}' ...")
...
```sql
-- dbt incremental model: only process new/changed rows, not a full table rebuild every run
...
Pitfall: a pipeline task that isn't idempotent (blind `INSERT` instead of `DELETE+INSERT` or
`MERGE` on a partition key) silently double-counts data on every retry or backfill — Airflow
*will* retry failed tasks, so idempotency isn't optional hardening, it's a correctness requirement
from day one.
