...
```go
// Worker pool with graceful shutdown via context cancellation
func RunPool(ctx context.Context, jobs <-chan Job, workers int) error {
	...
	return g.Wait() // first error cancels ctx for all other workers
}
```

Pitfall: a `select` with only a `case <-ctx.Done()` and a channel-read case will block forever if
the channel is never closed AND ctx is never cancelled — always have a clear owner responsible for
closing `jobs` (typically the producer, not the pool) or the goroutines leak silently.
...
Pitfall: forgetting to capture loop variables correctly in table-driven tests running in
parallel (`t.Parallel()` inside the subtest) causes all subtests to silently share the last
loop iteration's `tt` — Go 1.22+ fixed per-iteration loop var scoping, but pre-1.22 code needs
`tt := tt` shadowed inside the loop body before `t.Run`.
