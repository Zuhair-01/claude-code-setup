...
For deep Python code patterns (async, typing, project structure), use `python-patterns` instead — use this skill for language/tooling-level decisions.
...
```python
# Structural pattern matching -- destructure + guard in one expression (3.10+)
match command.split():
    case ["move", direction] if direction in {"up", "down", "left", "right"}:
        move(direction)
    case ["quit" | "exit"]:
        shutdown()
    case [cmd, *rest] if cmd in registry:
        registry[cmd](*rest)
    case _:
        raise ValueError(f"unknown command: {command}")

# Descriptor: validated attribute reused across many classes without repeating logic
class PositiveInt:
...
Pitfall: pattern matching's `case _:` (wildcard) is mandatory for exhaustiveness in anything
user-facing -- an unmatched case falls through silently otherwise, no error.
...
```bash
# uv: replaces pip+venv+pip-tools, resolves + installs in one pass, ~10-100x faster than pip
uv init myproject && cd myproject
uv add fastapi uvicorn
uv add --dev pytest ruff mypy
uv run pytest                 # runs inside the managed venv, no manual activation

# ruff: replaces black+isort+flake8 with one Rust-based tool
ruff check --fix .            # lint + autofix
ruff format .                 # formatting (black-compatible)
```

Pitfall: `uv.lock` must be committed (it's the reproducibility guarantee, same role as
`package-lock.json`) — don't `.gitignore` it like you would a `venv/` directory.
