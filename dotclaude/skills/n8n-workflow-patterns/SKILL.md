...
## Available n8n MCP Tools (use these, don't guess node names/schemas)

This session has live `mcp__n8n__*` tools connected — use them instead of
recalling node syntax from training data, which drifts as n8n ships new
node versions:

| Tool | When to use |
|---|---|
| `search_nodes` | Don't know the exact node name — search by keyword first |
| `get_node` | Confirm a node's actual parameters/schema before wiring it into a workflow |
| `search_templates` / `get_template` | Check the real n8n template library for a proven starting point before building from scratch |
| `validate_node` | Sanity-check a node's config before deploying — catches typos/wrong param names |
| `validate_workflow` | **Always run before activating a workflow**, not just after something breaks |
| `tools_documentation` | Full docs for any of the above if a call's behavior is unclear |

Local instance for this Empire: `localhost:5678` (per `Tech_Stack.md`,
monitored by Uptime Kuma once that's live — see
`Open_Source_Self_Host_Stack.md`). Workflows live at
`Empire_Base/n8n_workflows/`.
