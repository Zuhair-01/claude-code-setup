...
```dart
// Riverpod 2.x: async data with automatic loading/error states, no manual FutureBuilder
@riverpod
...
class OrdersView extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return ordersAsync.when(
      data: (orders) => ListView(children: orders.map(OrderTile.new).toList()),
...
Pitfall: calling `ref.watch()` inside a callback (`onPressed`, `onTap`) instead of `ref.read()`
subscribes the widget to rebuild on every state change even though the callback only runs once —
`watch` is for the `build()` method only; use `read` anywhere state is consumed once, not
