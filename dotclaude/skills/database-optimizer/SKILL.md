...
```sql
-- Read the plan before adding an index -- confirm the actual bottleneck first
EXPLAIN (ANALYZE, BUFFERS) SELECT * FROM orders WHERE customer_id = 42 AND status = 'pending';
-- Look for: Seq Scan on a large table (missing index), high "actual time" vs "estimated rows"
-- mismatch (stale statistics -> run ANALYZE orders), or Sort/Hash spilling to disk (work_mem too low)

-- Composite index: column order matters -- most-selective / most-frequently-filtered-alone first
CREATE INDEX idx_orders_customer_status ON orders (customer_id, status);
-- Serves: WHERE customer_id = ?  AND  WHERE customer_id = ? AND status = ?
-- Does NOT serve: WHERE status = ? alone (leftmost-prefix rule)
```

Pitfall: adding an index without checking `EXPLAIN` first is guessing — a "slow query" is often
actually a stale-statistics problem (`ANALYZE table_name` fixes it in seconds) or a `work_mem`
sort spill, and a new index adds real write-path cost (every INSERT/UPDATE maintains it) for zero
benefit if it doesn't match the actual query pattern.
...
```python
# N+1: this looks innocent but issues 1 query for authors + N queries (one per author) for books
for author in Author.objects.all():
    print(author.books.count())  # separate query EVERY iteration

# Fixed: prefetch_related batches the child query into ONE extra query total, not N
for author in Author.objects.prefetch_related("books"):
    print(author.books.count())  # already in memory, no query

# select_related (SQL JOIN, for *-to-one) vs prefetch_related (separate batched query, for *-to-many)
Order.objects.select_related("customer")       # one JOIN query
Author.objects.prefetch_related("books")        # two queries total: authors, then books WHERE author_id IN (...)
```

Pitfall: `select_related` on a *-to-many relationship silently produces a row-multiplied JOIN
result (one row per child), which duplicates the parent's fields across rows and breaks naive
counting — use `prefetch_related` for *-to-many, `select_related` only for *-to-one/foreign-key.
