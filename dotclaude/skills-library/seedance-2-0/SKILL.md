...
> **⚠️ Environment mismatch — not usable as-is on this machine.** This
> skill assumes an "OpenMontage" Python framework (`tools.tool_registry`,
> `video_selector`, `FAL_KEY`/`HEYGEN_API_KEY`/`REPLICATE_API_TOKEN` env
> vars) that is not installed in this Empire Base setup. The actual live
> Seedance 2.0 pipeline here is Playwright browser automation against
> higgsfield.ai — see **`seedance-auto-generate`** instead (it's the
> skill `higgsfield-image-auto` hands off to). Keep this file only as
> prompt-engineering reference (the "Higgsfield Methodology" prompt
> structure below is genuinely useful) — do not attempt to call
> `registry.get(...)` here, it will fail.
